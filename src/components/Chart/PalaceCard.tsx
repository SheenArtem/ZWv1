...
export const PalaceCard: FC<PalaceCardProps> = ({ data, className, isLiuNian, isLiuYue }) => {
    // Badges must be at least text-xs (12px line-height usually, but can be forced 12px)
    const getMutagenBadge = (m?: string) => {
        ...
        // text-[12px]
        return (
            <span className={clsx("ml-1 text-[12px] px-1 rounded border font-bold inline-block leading-tight", colorMap[m])}>
                {charMap[m]}
            </span>
        );
    };
    ...
    const MajorStarItem = ({ star }: { star: Star }) => (
        <div className="flex items-center gap-1 mb-1 flex-wrap">
            ...
            {star.brightness && (
                <span className="text-[12px] text-slate-400 font-normal leading-tight">
                    {star.brightness}
                </span>
            )}
    ...
    const GodItem = ({ name, color }: { name: string, color: string }) => {
        return (
            <span className={clsx("text-[12px] leading-tight mr-2", color)}>
                {name}
            </span>
        );
    };
    ...
    return (
        <div className={clsx(
            "relative bg-slate-9 border border-slate-700 p-1 flex flex-col h-48 sm:h-60 overflow-hidden bg-slate-800 hover:z-10 hover:overflow-visible hover:h-auto hover:shadow-2xl transition-all duration-200",
            className,
            isLiuNian && "ring-2 ring-inset ring-amber-500",
            isLiuYue && !isLiuNian && "ring-2 ring-inset ring-emerald-500"
        )}>
            {isLiuNian && (
                <div className="absolute top-0 right-0 bg-amber-600 text-[12px] px-1 text-white font-bold opacity-full z-20">
                    流年
                </div>
            )}
            {isLiuYue && (
                <div className="absolute top-0 right-10 bg-emerald-600 text-[12px] px-1 text-white font-bold opacity-full z-20">
                    流月
                </div>
            )}
            ...
            <div className="flex h-full w-full">
                <div className="w-3/5 flex flex-col pt-5 pl-1">
                    {data.majorStars.map(s => <MajorStarItem key={s.name} star={s} />)}
                </div>
                <div className="w-2/5 flex flex-col pt-5 pr-1 items-end text-right flex-wrap content-end">
                    {data.minorStars.map(s => <MinorStarItem key={s.name} star={s} />)}
                </div>
            </div>

            {/* Bottom Area */}
            {/* Increased padding-top to ensure separation */}
            <div className="absolute bottom-0 w-full flex flex-col px-1 pb-1 bg-gradient-to-t from-slate-900 via-slate-900 to-transparent pt-6">
                ...
                <div className="text-[12px] text-slate-500 font-mono leading-tight h-auto min-h-[1.2em] overflow-hidden text-center whitespace-normal break-words opacity-70 mb-1">
                    {data.xiaoXian}
                </div>

                <div className="flex justify-between items-end mt-1 border-t border-slate-700 pt-1">
                    <span className="text-[14px] font-bold text-amber-500 font-mono leading-none">
                        {data.daXian}
                    </span>
